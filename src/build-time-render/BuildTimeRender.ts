...
${blockCacheEntry}`
	...
	public runPath(callback: Function, path: string, output: string, jsonpName: string) {
		...
		if (paths.indexOf(path) === -1) {
			const pathExists = existsSync(join(this._output, ...path.split('/'), 'index.html'));
			if (pathExists) {
				console.warn(
					`On demand BTR: The page for '(${path})' already exists, but was not generated by BTR. Skipping.`
				);
				return callback();
			}
			console.warn(
			...
			outputFileSync(join(this._output, 'btr-manifest.json'), JSON.stringify([...paths, path], null, 4), 'utf8');
		}
		this._initialBtr = false;
